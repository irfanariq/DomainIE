...
public class ClassifierProgress extends javax.swing.JFrame {
    ...
    public ClassifierProgress(ClassifierPipeline classifierPipeline) {
        this.classifierPipeline = classifierPipeline;
        initComponents();
    }
    ...
    private void showProgressLabel() {
        counter++;
        int totalCurrentlyDocumentsDataprocessed = classifierPipeline.getCurrentlyClassifiedDocuments();
        int totalDocumentsToBeDataprocessed = classifierPipeline.getTotalDocumentsToBeClassified();
        ...
        if(!classifierPipeline.isCombining()) jLabel1.setText("Running Classifier");

        
        if (classifierPipeline.getCurrentlyRunningClassifier() != null) {
            dataprocessorName = ((Classifier)classifierPipeline.getCurrentlyRunningClassifier()).getClassifierHandler().getPluginName();
        }

        if (totalDocumentsToBeDataprocessed > 0 && totalCurrentlyDocumentsDataprocessed == totalDocumentsToBeDataprocessed) {
            dataprocessingDocumentsProgressLabel.setText("Classifying Completed. Loading" + trail);
        } else {
        ...
        int totalProcessedDataprocessor = classifierPipeline.getTotalProcessedClassifier();
        int totalDataprocessor = classifierPipeline.getNumberOfClassifiers();

        if (dataprocessorName.equalsIgnoreCase("")) {
            dataprocessingPipelineProgressLabel.setText("Setting up classifiers" + trail + StringUtils.repeat(" ", 40));
        } else {
